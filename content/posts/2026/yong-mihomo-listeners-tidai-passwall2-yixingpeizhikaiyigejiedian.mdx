...
date: '2026-05-16'
summary: >-
  Mihomo（Clash.Meta）原生支持 `listeners` 功能，可以为每个代理节点开一个独立的SOCKS5/HTTP
  端口。这意味着你不再需要安装 Passwall2 之类的额外插件——只需在`config.yaml` 里加几行
  YAML，就能实现「每个节点一个端口，即连即用」的效果。本文从原理、配置到使用，三步讲清楚。
tags:
  - 教程
draft: false
cover: ''
